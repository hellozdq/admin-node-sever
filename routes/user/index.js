...
router.post("/list", (req, res) => {
  ...
  let sql = `SELECT SQL_CALC_FOUND_ROWS u.id,a.account,a.id accountId,u.name,u.phone,u.roles FROM user u JOIN account a on u.relId=a.id  WHERE 1=1`;
  if (name) {
...
function handleRoles(arr, obj, index) {
  ...
  if (activeIndex !== -1) {
    if (arr[activeIndex].children) {
      arr[activeIndex].children = handleRoles(
        arr[activeIndex].children,
        obj,
        index + 1
      );
    }
